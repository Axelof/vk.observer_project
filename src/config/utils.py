...
def get_config(string: Union[str, bytes]) -> ConfigModel:
    ...
    if config.logging.log:
        enable_file_logging(time.strftime(config.logging.log_path), level=10 if config.logging.log_disable_debug else 20,
                            format=log_format, rotation="5 MB", compression="zip", retention="1 days")

    if config.logging.log_errors:
        enable_file_logging(time.strftime(config.logging.log_errors_path), level=30,
                            format=log_format, rotation="5 MB", compression="zip", retention="15 days")
